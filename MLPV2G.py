...
class MLPOneStepRegressor(nn.Module):
    """
    MLP baseline for one-step regression.
    Flatten input [lookback,1] -> [lookback] and predict next value.
    """
    def __init__(self, lookback: int, hidden_dims: Tuple[int, ...] = (128, 64), dropout: float = 0.1):
        super().__init__()
...
def main():
    ...
    model = MLPOneStepRegressor(lookback=lookback, hidden_dims=(128, 64), dropout=0.1).to(device)
